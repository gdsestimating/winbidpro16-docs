...
### 1. The `Counted` Handling

`Counted` parts are typically hardware parts such as screws or hinges, things you count and do not measure. The `UnitsNeeded` is a simple count of how many are needed. `PackagesToOrder` = `Ceiling(UnitsNeeded / UnitsPerPackage)`
...
### 2. The `Measured` Handling
`Measured` parts have a length. For example gasket material sold in rolls of 500'. For these line items, the `UnitsNeeded` are in inches **(though our program displays them in feet)** and `PackagesToOrder` = `Ceiling(UnitsNeeded / (Length * UnitsPerPackage))`.
